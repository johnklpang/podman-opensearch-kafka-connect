...
Dashboards must use **`http://opensearch:9200`** on the Podman network. Env-only fixes are unreliable; we mount:

`configs/opensearch-dashboards/opensearch_dashboards.yml`

```bash
./scripts/fix-dashboards-hosts.sh
# expect in container config:
...
```bash
# Ctrl+C any running 06-register-connector.sh first
./scripts/FIX-IT-NOW.sh
```

This installs **Aiven 3.1.1** (`OpensearchSinkConnector`), bakes a readable image, starts Connect with a correct `plugin.path`, and registers the sink.
